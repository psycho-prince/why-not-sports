...
export default async function MatchDetail({ params, searchParams }: PageProps<{ id: string }, { sport: string }>) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 pt-12 pb-40">
      <Link href="/" className="flex items-center text-zinc-600 hover:text-white transition mb-12 text-[10px] font-black uppercase tracking-[0.2em]">
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-[3.5rem] p-12 md:p-20 text-center relative">
        <h1 className="text-4xl md:text-6xl font-black italic uppercase leading-none mb-10 tracking-tighter text-white">
          {searchParams.sport === 'cricket' ? match.data?.name : (match.events ? match.events[0].strEvent : "Match Details")}
        </h1>
        <div className="flex justify-center items-center py-12 border-y border-zinc-800/50 mb-10">
           {searchParams.sport === 'cricket' ? (
        ...
        <p className="text-green-500 font-black uppercase tracking-widest text-sm">
          {searchParams.sport === 'cricket' ? match.data?.status : (match.events ? match.events[0].strProgress : "Live Updates Only")}
        </p>
      </div>
    </div>
  );
